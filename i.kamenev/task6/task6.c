...
void print_line(int fd, textTable *table, int lineNum) {
...
int main(int argc, char *argv[]) {
    ...
    if (build_line_table(fName, &indTable, &linesNum) != 0) {
    ...
    while (1) {
        ...
        if (lineNum == 0) {
            break;
        } else if (lineNum > linesNum) {
            printf("Invalid line number.\n");
        } else if(lineNum > 0 && lineNum <= linesNum){
            print_line(fName, indTable, lineNum - 1);
        }
    }
